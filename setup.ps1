...
npm install express nodemailer smtp-server imap-server sqlite3 bcrypt express-session passport passport-local

# Verify installation
if ($LASTEXITCODE -eq 0) {
    Write-Host "Setup completed successfully!" -ForegroundColor Green
    Write-Host "Directories created: data, public, views, src" -ForegroundColor Green
    Write-Host "Dependencies installed: express, nodemailer, smtp-server, imap-server, sqlite3, bcrypt, express-session, passport, passport-local" -ForegroundColor Green
    Write-Host ""
